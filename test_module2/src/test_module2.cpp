#include "test_module2/test_module2.h"
...
void TestModule2::initialize(const int control_cycle_msec, robotis_framework::Robot *robot)
{
  ...
  queue_thread_ = boost::thread(boost::bind(&TestModule2::queueThread, this));
}

void TestModule2::queueThread()
{
  ros::NodeHandle ros_node;
  ...
  {
    /* subscribe topics */
    ros::Subscriber imu_sub = ros_node.subscribe("/robotis_op3/torso_orientation", 5, &TestModule2::torsoIMUMsgCallback, this);
  }

  ros::Subscriber amplitude_sub = ros_node.subscribe("/adol/op3/test/amplitude", 5, &TestModule2::amplitudeCallback, this);
  ros::Subscriber period_sub = ros_node.subscribe("/adol/op3/test/period", 5, &TestModule2::periodCallback, this);
...
void TestModule2::torsoIMUMsgCallback(const geometry_msgs::Pose::ConstPtr &msg)
{
  torso_pose_.position = msg->position;
  torso_pose_.orientation = msg->orientation;
}

void TestModule2::amplitudeCallback(const std_msgs::Float64::ConstPtr &msg)
{
  amplitude_ = msg->data;
}

void TestModule2::periodCallback(const std_msgs::Float64::ConstPtr &msg)
{
  period_ = msg->data;
}

void TestModule2::stop()
{

}

bool TestModule2::isRunning()
{
  return false;
}

void TestModule2::onModuleEnable()
{
  current_time_sec_ = 0;
}

void TestModule2::onModuleDisable()
{

}

void TestModule2::updateJointAngles(std::map<std::string, robotis_framework::Dynamixel *> dxls)
{
...
void TestModule2::computeForwardKinematics(void)
{
...
double TestModule2::getHipHeight(void)
{
...
void TestModule2::process(std::map<std::string, robotis_framework::Dynamixel *> dxls, std::map<std::string, double> sensors)
{
  if (enable_ == false)
    return;
  ...
  Eigen::Matrix4d mat_G_to_Pelvis = robotis_framework::getTransformationXYZRPY(0, 0, default_hip_height_ + 0.03, 0.1*sin(2.0 * M_PI * current_time_sec_/ period_), 0, 0);
  Eigen::Matrix4d mat_G_to_RF = robotis_framework::getTransformationXYZRPY(0, -0.5*(op3_kd_->leg_side_offset_m_), 0, 0, 0, 0);
  Eigen::Matrix4d mat_G_to_LF = robotis_framework::getTransformationXYZRPY(0,  0.5*(op3_kd_->leg_side_offset_m_), 0, 0, 0, 0);
